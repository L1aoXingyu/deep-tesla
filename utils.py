...
def ffmpeg_frame_count(path):
    cmd = 'ffmpeg -i {} -vcodec copy -acodec copy -f null /dev/null2>&1'\
        .format(path)
...
def mkv_to_mp4(mkv_path, remove_mkv=False):
    ...
    cmd = 'ffmpeg -i {} -c:v copy -c:a libfdk_aac -b:a 128k {} >/dev/null2>&1'\
        .format(mkv_path, mp4_path)
    sp.call(cmd, shell=True)

    assert os.path.isfile(mp4_path)
    # make sure that the file got generated successfully
...
def get_human_steering(epoch_id):
    ...
    steering_path = join_dir(epoch_dir,
...
def visualize(epoch_id, machine_steering, out_dir, perform_smoothing=False,
              verbose=False, verbose_progress_step=100,
